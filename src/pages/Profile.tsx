        ...
        <nav className="flex-1 px-4 space-y-2 overflow-y-auto">
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-5 h-5 ${isActive ? "text-[#8B5A2B]" : "text-gray-400"}`}
                />
                {tab.label}
              </button>
            );
      ...
      <main className="flex-1 p-4 md:p-10 overflow-y-auto">
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 md:mb-8 gap-4">
            {(activeTab === "personal" || activeTab === "dashboard") &&
              !isEditing && (
                <button
                ...
                <div className="mt-1 space-y-1">
                  {rejectedProperties.map((prop) => (
                    <p key={prop.id} className="text-xs text-red-700">
                      ...
                      {prop.rejectionReason || "No reason provided by admin"}
                    </p>
                  ))}
                </div>
              </div>
...
function NewBookingsTab() {
  ...
  const fetchBookings = async () => {
    if (!user) return;
    try {
      const allOwnerBookings = await bookingService.getBookingsByOwner(
        user.uid,
      );
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start gap-2 mb-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="min-w-0">
                      <h3 className="font-bold text-[#1A1A2E] text-xs leading-tight truncate">
                        {booking.visitorName}
                      </h3>
                  ...
                  <div className="sm:text-right flex items-center sm:block gap-2 sm:gap-0 mt-1 sm:mt-0">
                    <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest leading-none mb-0.5 whitespace-nowrap">
                      Payout
                    </p>
                ...
                <div className="flex items-center gap-1 mb-4 text-[10px] font-medium text-slate-500 whitespace-nowrap overflow-hidden text-ellipsis">
                  ...
                  <span className="truncate">
                    {booking.property?.title ||
                      (booking as any).propertyTitle ||
                      (booking as any).propertyName}
                  </span>
                </div>
              </div>
        ...
        {selectedBooking && !showRejectModal && !showConfirmModal && (
          <div className="modal-overlay">
            ...
            <motion.div
              ...
              className="modal-content bg-white rounded-3xl p-8 max-w-lg shadow-2xl relative z-10 border border-slate-100"
            >
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  ...
                  />
                  <div>
                    <h4 className="font-black text-lg text-[#1A1A2E] leading-tight">
                      {selectedBooking.visitorName}
                    </h4>
                    <p className="text-sm text-slate-500 font-medium">
                      {selectedBooking.visitorContact}
                    </p>
                ...
                {/* Guest List */}
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 ml-1">
                    Guest Occupants ({selectedBooking.guests?.length || 1})
                  </p>
                  <div className="space-y-2">
                    {(
                      selectedBooking.guests || [
                        {
                          name: selectedBooking.visitorName,
                          age: "Adult",
                ...
                {selectedBooking.status === "pending" && (
                  <div className="flex gap-3 pt-4">
                    <button
                      ...
                      className="flex-1 py-4 bg-red-50 hover:bg-red-100 text-red-500 rounded-2xl font-bold transition-all border border-red-100"
                    >
                      Reject
                    </button>
                    <button
                      onClick={() => setShowConfirmModal(true)}
                      className="flex-[2] py-4 bg-emerald-500 hover:bg-emerald-600 text-white rounded-2xl font-bold transition-all shadow-lg shadow-emerald-500/20"
                    >
                      Confirm Visit
                    </button>
                  </div>
        ...
        {showRejectModal && (
          <div className="modal-overlay">
            ...
            <motion.div
              ...
              className="modal-content bg-white rounded-3xl p-8 max-w-md shadow-2xl relative z-[120]"
            >
        ...
        {showConfirmModal && (
          <div className="modal-overlay">
            ...
            >
              ...
              <p className="text-sm text-gray-500 mb-6">
                Schedule a property visit for the visitor. Please select a date
                and time.
              </p>
...
function PersonalInfoTab({
  ...
  useEffect(() => {
    const fetchRecentBookings = async () => {
      if (!user) return;
      try {
        const userBookings = await bookingService.getBookingsByVisitor(
          user.uid,
        );
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-6">
            ...
            <button className="text-sm font-bold text-[#F59E0B] hover:text-amber-600">
              View All
            </button>
          </div>
                ...
                <div key={booking.id} className="flex gap-4 items-center">
                  <img
                    src={
                      ...
                      "https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?auto=format&fit=crop&q=80&w=100"
                    }
                    alt="Stay"
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 overflow-x-auto">
          ...
          <div className="space-y-4 min-w-[500px]">
            <div className="grid grid-cols-4 gap-4 text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">
              <span>Date & Time</span>
              <span>Property</span>
                ...
                >
                  ...
                  <span className="text-gray-500 text-xs">
                    {booking.checkIn && booking.checkOut
                      ? `${format(booking.checkIn.toDate ? booking.checkIn.toDate() : new Date(booking.checkIn), "MMM dd")} - ${format(booking.checkOut.toDate ? booking.checkOut.toDate() : new Date(booking.checkOut), "MMM dd")}`
                      : "Unknown"}
                  </span>
                  <span className="font-bold text-[#1A1A2E] text-right">
                    ₹
                    {(
                      booking.totalAmount ||
...
function MyBookingsTab() {
  ...
  const getVisitorStatusDisplay = (status: string, checkOut: Date | null) => {
    ...
    if (status === "rejected_by_owner")
      return {
        label: "Cancelled",
        note: "Cancelled by property owner. 95% refund credited to your wallet.",
        color: "bg-rose-500/90",
      };
    if (status === "cancelled")
      return { label: "Cancelled by you", note: "", color: "bg-gray-500/90" };
    return {
      ...
      async (snapshot) => {
        try {
          ...
          bookingsWithProperties.forEach(async (booking) => {
            if (
              booking.status === "confirmed" &&
              !ownersInfo[booking.ownerId]
            ) {
              setLoadingOwners((prev) => ({
                ...prev,
                [booking.ownerId]: true,
              }));
              try {
      ...
      {bookings.length === 0 ? (
        <div className="bg-white rounded-3xl p-12 shadow-sm border border-gray-100 text-center">
          ...
          <p className="text-gray-500 max-w-xs mx-auto text-sm leading-relaxed">
            Your travel history is empty. Start exploring properties to make
            your first stay unforgettable!
          </p>
          <button
                ...
                <div className="flex flex-col lg:flex-row">
                  ...
                  <div className="lg:w-1/3 relative h-64 lg:h-auto overflow-hidden">
                    <img
                      src={
                        booking.property?.photos?.[0] ||
                        `https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?auto=format&fit=crop&q=80&w=600&h=400`
                      }
                      alt="Property"
                    ...
                    <div className="absolute top-6 left-6 flex flex-col gap-2 items-start">
                      {(() => {
                        const display = getVisitorStatusDisplay(
                          booking.status,
                          booking.checkOut,
                        );
                        return (
                          <>
                            <span
                              className={`text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-full shadow-2xl backdrop-blur-xl border border-white/20 ${display.color}`}
                            >
                              {display.label}
                            </span>
                            {display.note && (
                  ...
                  <div className="flex-1 p-8 lg:p-10 flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start mb-6">
                        <div>
                          <h3 className="font-black text-[#1A1A2E] text-2xl lg:text-3xl mb-2 tracking-tight">
                            {booking.property?.title || "Unknown Property"}
                          </h3>
                        ...
                        <div className="text-right">
                          ...
                          {booking.status === "rejected_by_owner" && (
                            <div className="mt-2 bg-red-50 text-red-600 px-3 py-1 rounded-lg border border-red-100 text-[9px] font-bold text-center">
                              Booking rejected by owner. 95% refund credited to
                              wallet.
                            </div>
                          )}
                          ...
                          {isLoadingOwner ? (
                              ...
                              <div className="flex items-center gap-4 flex-1">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">
                                    {owner.phone || "Contact not listed"}
                                  </p>
                                </div>
                              </div>
                            ...
                            <div className="p-4 bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-center">
                              <p className="text-sm text-slate-400 italic">
                                Host information will be available shortly.
                              </p>
                            </div>
                          )}
                        </div>
                    ...
                    {/* Action Suite */}
                    {!["cancelled", "rejected_by_owner", "completed"].includes(
                      booking.status,
                    ) && (
                      <div className="flex flex-wrap items-center gap-4 pt-4 border-t border-slate-50">
                        {(() => {
                          ...
                          if (checkOut && now > checkOut) {
                            return (
                              ...
                              >
                                <Star
                                  className={`w-4 h-4 ${hasReviewed ? "fill-amber-200" : "fill-amber-400 text-amber-400"}`}
                                />
                        ...
                        {isValidGoogleMapsLink(
                          booking.property?.googleMapsLink,
                        ) && (
                          <button
                            onClick={() =>
                              window.open(
...
function CancellationModal({
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      if (refundAmount > 0) {
        showToast(
          `Booking cancelled. ₹${refundAmount.toLocaleString()} (${refundPercentage}%) refund initiated to your wallet.`,
          "success",
        );
      } else {
            ...
            <div className="space-y-6">
              <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 flex gap-3">
                ...
                <p className="text-sm text-amber-800">
                  Please tell us why you want to cancel your stay at{" "}
                  <span className="font-bold">{booking.property?.title}</span>.
                </p>
              </div>
              <textarea
                value={reason}
          ...
          {step === 2 && (
            <div className="space-y-6">
                      ...
                      <>
                        <div className="p-4 bg-white rounded-xl border border-slate-100 shadow-sm">
                          ...
                          <p className="mt-2 text-[10px] text-slate-400 italic">
                            Calculated based on {hoursUntilCheckIn.toFixed(1)}{" "}
                            hours remaining until check-in.
                          </p>
                        </div>
                        <div className="p-4 bg-amber-50/50 rounded-xl border border-amber-100 mt-4">
                          <p className="text-[11px] text-amber-800 font-medium leading-relaxed">
                            Note: Refund will be credited to your ShelterBee
                            wallet within 5-10 business days. Platform service
                            charges may be retained. Once cancelled, this
                            booking cannot be restored. By confirming you accept
                            ShelterBee's Cancellation and Refund Policy.
                          </p>
                        </div>
                      </>
                    );
              ...
              <label className="flex items-start gap-3 cursor-pointer group">
                <div
                  className={`mt-1 w-5 h-5 rounded border flex items-center justify-center transition-colors ${agreed ? "bg-amber-500 border-amber-500" : "bg-white border-gray-300 group-hover:border-amber-500"}`}
                >
                  {agreed && (
                ...
                <span className="text-sm text-gray-600 leading-relaxed">
                  I have read and agree to ShelterBee's Terms of Use, Privacy
                  Policy, and applicable platform policies.
                </span>
              </label>
...
function PaymentsTab() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const refundTxns = txns.filter(
        (t) => t.type === "credit" && t.reason === "refund",
      );

      const enhancedRefunds = await Promise.all(
        refundTxns.map(async (t) => {
          if (t.bookingId) {
  ...
  const handleWithdraw = async () => {
    ...
    if (
      withdrawType === "bank" &&
      (!bankDetails.accountNumber || !bankDetails.bankName || !bankDetails.ifsc)
    ) {
    ...
    try {
      const details =
        withdrawType === "upi"
          ? {
              accountNumber: bankDetails.upiId,
              bankName: "UPI Transfer",
              ifsc: "UPI",
              branchName: "Online",
              accountHolderName: user?.displayName || "Visitor",
            }
          : {
              ...bankDetails,
              branchName: "Pending",
      ...
      showToast(
        "Withdrawal request submitted. Amount will be credited in 3-4 working days.",
        "success",
      );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-gray-500 mb-1">
              Wallet Balance (Refunds collected)
            </p>
                ...
                {refunds.map((refund) => (
                  ...
                  >
                    <td className="py-4 px-4 text-sm text-gray-500 font-medium">
                      {refund.createdAt
                        ? format(refund.createdAt.toDate(), "MMM dd, yyyy")
                        : "Unknown"}
                    </td>
              ...
              <tbody>
                {bookings.map((booking) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-4 text-sm font-bold text-[#1A1A2E]">
                      {booking.property?.title || "Unknown Property"}
                    </td>
                    <td className="py-4 px-4 text-sm text-gray-500">
                      {booking.checkIn && booking.checkOut
                        ? `${format(booking.checkIn.toDate ? booking.checkIn.toDate() : new Date(booking.checkIn), "MMM dd")} - ${format(booking.checkOut.toDate ? booking.checkOut.toDate() : new Date(booking.checkOut), "MMM dd, yyyy")}`
                        : "Unknown"}
                    </td>
          ...
          <div className="modal-overlay">
            ...
            >
              ...
              <p className="text-gray-500 mb-6 font-medium">
                Collect your refunds into your bank account or UPI.
              </p>
...
function FavouritesTab() {
  ...
  const [activeSubTab, setActiveSubTab] = useState<
    "live" | "approved" | "rejected" | "hidden" | "pending"
  >("live");
    ...
    switch (activeSubTab) {
      case "live":
        filtered = allProperties.filter((prop) =>
          propertyService.isPropertyAvailable(prop, now),
        );
        break;
      case "approved":
      ...
      case "hidden":
        filtered = allProperties.filter(
          (prop) => prop.availabilityStatus === "unavailable",
        );
        break;
      case "pending":
  ...
  const [selectedPropertyToHide, setSelectedPropertyToHide] =
    useState<any>(null);
  const [hideOption, setHideOption] = useState<"today" | "manual" | "range">(
    "manual",
  );
  ...
  const handleHideProperty = async () => {
    ...
    const toDateStr =
      hideOption === "manual" ? "I make it available" : to || "today";

    showConfirm(
      `Your property will be unavailable and will not visible to others publicly till ${toDateStr}. Are you sure?`,
      async () => {
        try {
  ...
  const viewBookings = async (propertyId: string, title: string) => {
    ...
    try {
      ...
      const propBookings = allOwnerBookings.filter(
      ...
      const bookingsWithFinancials = await Promise.all(
        propBookings.map(async (booking) => {
          const financials = await bookingService.getBookingFinancials(
            booking.id!,
          );
  ...
  const submitReply = async (reviewId: string) => {
    ...
    try {
      ...
      setSelectedPropertyReviews((prev) =>
        prev.map((r) =>
          r.id === reviewId ? { ...r, reply: replyText[reviewId] } : r,
        ),
      );
  ...
  const handleRejectBooking = async () => {
    ...
    try {
      ...
      const amount =
        rejectionBooking.totalAmount || rejectionBooking.estimatedCost || 0;
      ...
      // 3. Notify Visitor
      const visitor = await userService.getUserProfile(
      ...
      showToast(
        "Booking rejected and refund processed successfully",
        "success",
      );
  ...
  const updateBookingStatus = async (
    ...
    try {
      ...
      const propBookings = allOwnerBookings.filter(
        (b) => b.propertyId === selectedPropertyBookings[0]?.propertyId,
      );
  ...
  return (
    <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 font-bold uppercase tracking-widest">
                  Control your property presence
                </p>
              </div>
            </div>
            <button
      ...
      {properties.length === 0 ? (
        ...
        <div
          className={`grid grid-cols-1 ${isOwner ? "md:grid-cols-2 lg:grid-cols-3" : "sm:grid-cols-2 lg:grid-cols-3"} gap-6`}
        >
              ...
              >
                <div className="relative h-40 md:h-44 overflow-hidden">
                  <img
                    src={
                      property.photos?.[0] ||
                      "https://images.picsum.photos/seed/property/600/400"
                    }
                    alt={property.title}
                  ...
                  <div className="absolute top-2 right-2 flex flex-col gap-1 items-end">
                    <div
                      className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest shadow-lg backdrop-blur-md border border-white/20 ${
                ...
                <div className="p-4 flex flex-col flex-1">
                  <div className="mb-3">
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-1 mb-1">
                      <h3 className="font-bold text-[#1A1A2E] text-sm tracking-tight truncate w-full sm:flex-1 group-hover:text-amber-600 transition-colors leading-tight">
                        {property.title || "Property"}
                      </h3>
                      <p className="text-sm font-black text-[#F59E0B] shrink-0">
                        ₹{(property.pricePerDay || 0).toLocaleString()}
                  ...
                  {property.status === "Rejected" && (
                    <div className="mb-3 p-2.5 bg-red-50 border border-red-100 rounded-xl flex gap-2">
                      ...
                      <p className="text-[9px] text-red-700 font-medium leading-tight line-clamp-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-auto">
                    {property.status === "Rejected" ? (
                      ...
                      <>
                        ...
                        <button
                          onClick={() =>
                            viewBookings(property.id, property.title)
                          }
                        ...
                        <button
                          onClick={() =>
                            viewReviews(property.id, property.title)
                          }
        ...
        {showHideModal && (
          <div className="modal-overlay">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 20 }}
              className="modal-content bg-white rounded-3xl p-8 max-w-md shadow-2xl relative z-10 border border-slate-100"
            >
              <h3 className="text-2xl font-bold text-[#1E1B4B] mb-6 text-center">
                Hide Property
              </h3>

              <div className="space-y-4 mb-8">
                <p className="text-sm text-gray-500 mb-4">
                  Select how long you want to hide this property from the
                  website.
                </p>

                <label
                  className={`flex items-center gap-3 p-4 border rounded-xl cursor-pointer transition-all ${hideOption === "today" ? "border-amber-500 bg-amber-50" : "border-gray-200 hover:bg-gray-50"}`}
                >
                  <input
                    type="radio"
                    name="hideOption"
                    checked={hideOption === "today"}
                    onChange={() => setHideOption("today")}
                ...
                <label
                  className={`flex items-center gap-3 p-4 border rounded-xl cursor-pointer transition-all ${hideOption === "manual" ? "border-amber-500 bg-amber-50" : "border-gray-200 hover:bg-gray-50"}`}
                >
                  <input
                    type="radio"
                    name="hideOption"
                    checked={hideOption === "manual"}
                    onChange={() => setHideOption("manual")}
                    className="w-4 h-4 text-amber-500"
                  />
                  <span className="font-medium text-sm">
                    Until I make it available
                  </span>
                </label>

                <label
                  className={`flex items-center gap-3 p-4 border rounded-xl cursor-pointer transition-all ${hideOption === "range" ? "border-amber-500 bg-amber-50" : "border-gray-200 hover:bg-gray-50"}`}
                >
                  <input
                    type="radio"
                    name="hideOption"
                    checked={hideOption === "range"}
                    onChange={() => setHideOption("range")}
                    className="w-4 h-4 text-amber-500"
                  />
                  <span className="font-medium text-sm">
                    Specific date range
              ...
              <div className="flex gap-4">
                ...
                <button
                  ...
                  disabled={
                    hideOption === "range" &&
                    (!unavailabilityFrom || !unavailabilityTo)
                  }
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-extrabold text-[#1E1B4B]">
                  Reviews for {selectedPropertyTitle}
                </h3>
                <button
                  onClick={() => setShowReviewsModal(false)}
        ...
        {showBookingsModal && (
          <div className="modal-overlay p-4">
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-extrabold text-[#1E1B4B]">
                  Bookings for {selectedPropertyTitle}
                </h3>
                <button
                  onClick={() => setShowBookingsModal(false)}
                  className="text-gray-400 hover:text-gray-600"
              ...
              {loadingBookings ? (
                ...
                <div className="space-y-4">
                  {selectedPropertyBookings.map((booking) => (
                    <div
                      key={booking.id}
                      className="border border-gray-100 rounded-xl p-4 flex flex-col md:flex-row justify-between gap-4"
                    >
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          Contact: {booking.visitorContact}
                        </p>
                        <p className="text-sm text-gray-500">
                          {booking.checkIn
                            ? format(booking.checkIn, "MMM dd, yyyy")
                        ...
                        {booking.guests && booking.guests.length > 0 && (
                          <div className="mt-3 p-3 bg-slate-50 rounded-lg border border-slate-100">
                            <p className="text-xs font-bold text-[#1E1B4B] uppercase tracking-wider mb-2">
                              Guest Details
                            </p>
                            <div className="space-y-2">
                              {booking.guests.map((guest: any, idx: number) => (
                                ...
                                >
                                  ...
                                  <span className="text-gray-400">
                                    {guest.relation || guest.contact || "Guest"}
                                  </span>
                                </div>
                              ))}
                            </div>
        ...
        {showRejectionModal && (
          <div className="modal-overlay p-4">
            <motion.div
              ...
              className="absolute inset-0 bg-black/60 backdrop-blur-sm"
              onClick={() =>
                !isProcessingRejection && setShowRejectionModal(false)
              }
            />
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
            ...
            >
              <h3 className="text-2xl font-extrabold text-[#1E1B4B] mb-2 text-center">
                Reject Booking
              </h3>
              <p className="text-sm text-gray-500 mb-6 text-center">
                Please provide a reason for rejecting this booking. The visitor
...
function WalletTab({
  ...
  const handleSaveBankDetails = async () => {
    ...
    try {
      await walletService.updateBankAccount(user!.uid, {
        ...bankDetails,
        verified: true,
      });
            ...
            {walletTransactions
              .filter((txn) => {
                if (isVisitor)
                  return (
                    txn.type === "credit" &&
                    (txn.refundPercentage || txn.paymentPartnerCharge)
                  );
                return true;
              })
              .map((txn) => (
                    ...
                    <div className="mt-4 pt-4 border-t border-dashed border-slate-100 flex flex-col gap-2">
                      ...
                      <div className="flex justify-between items-center text-[10px]">
                        ...
                        <span className="font-bold text-emerald-600">
                          {txn.refundPercentage}%
                        </span>
                      </div>
                      {txn.paymentPartnerCharge > 0 && (
                        <div className="flex justify-between items-center text-[10px]">
                          <span className="font-bold text-slate-400 uppercase">
                            Payment Partner Charge (5%)
                          </span>
                          <span className="font-bold text-red-400">
                            -₹{txn.paymentPartnerCharge.toLocaleString()}
                          </span>
                        </div>
                      )}
                    </div>
      ...
      <div className="space-y-8">
        <section>
          <h3 className="text-lg font-bold text-[#1A1A2E] mb-3 flex items-center gap-2">
            <ShieldCheck className="w-5 h-5 text-[#F59E0B]" /> Terms &
            Conditions
          </h3>
          <p className="text-sm text-gray-600 leading-relaxed">
            By using ShelterBee, you agree to ShelterBee's Terms of Use, Privacy
            Policy, and applicable platform policies. All bookings are subject
            to host approval and availability. Users must provide accurate
            identification when requested. ShelterBee reserves the right to
            suspend accounts that violate our community guidelines.
          </p>
        </section>

        <section>
          <h3 className="text-lg font-bold text-[#1A1A2E] mb-3 flex items-center gap-2">
            <CreditCard className="w-5 h-5 text-[#F59E0B]" /> Payment & Refund
            Policy
          </h3>
          <p className="text-sm text-gray-600 leading-relaxed">
            Payments are securely processed via our payment partners. A platform
            fee is applied to all bookings. Refunds for cancellations are
            subject to the specific property's cancellation policy. The platform
        ...
        <section>
          <h3 className="text-lg font-bold text-[#1A1A2E] mb-4 flex items-center gap-2">
            <HelpCircle className="w-5 h-5 text-[#F59E0B]" /> Frequently Asked
            Questions
          </h3>
          <div className="space-y-4">
            <div className="p-4 bg-gray-50 rounded-xl">
              <h4 className="font-bold text-[#1A1A2E] text-sm mb-2">
                Is it safe to book?
              </h4>
              <p className="text-sm text-gray-600">
                Yes, stays are verified before approval and basic safety checks
                are done to ensure your security.
              </p>
            </div>
            <div className="p-4 bg-gray-50 rounded-xl">
              <h4 className="font-bold text-[#1A1A2E] text-sm mb-2">
              ...
              <p className="text-sm text-gray-600">
                You can retry or use another method. Your booking is not
                confirmed until the payment succeeds.
              </p>
            </div>
            <div className="p-4 bg-gray-50 rounded-xl">
              <h4 className="font-bold text-[#1A1A2E] text-sm mb-2">
              ...
              <p className="text-sm text-gray-600">
                No hidden charges. The total price is shown before you make the
                payment.
              </p>
            </div>
          </div>
    ...
    <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
      {properties.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">
            You have no properties waiting for admin approval.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              <div className="relative h-48">
                ...
                <div className="absolute top-3 right-3 flex gap-2">
                  <span
                    className={`text-xs font-bold px-2 py-1 rounded-md uppercase tracking-wider ${property.status === "Rejected" ? "bg-red-500" : "bg-amber-500"} text-white`}
                  >
                    {property.status === "Rejected"
                      ? "Rejected"
                      : "Pending Review"}
              ...
              <div className="p-4">
                <h3 className="font-bold text-[#1A1A2E] text-lg mb-1 truncate">
                  {property.title}
                </h3>
                ...
                <div className="border-t border-gray-100 pt-4">
                  {property.status === "Rejected" ? (
                    ...
                    <p className="text-sm text-amber-600 bg-amber-50 p-3 rounded-lg border border-amber-100">
                      This property is currently under review by our admin team.
                      It will be visible to visitors once approved.
                    </p>
                  )}
                </div>
              </div>
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        const totalReviewsCount = properties.reduce(
          (sum, p: any) => sum + (p.totalReviews || p.reviewCount || 0),
          0,
        );
        const weightedSum = properties.reduce(
          (sum, p: any) =>
            sum +
            (p.averageRating || p.rating || 0) *
              (p.totalReviews || p.reviewCount || 0),
          0,
        );
        const aggregateAverageRating =
          totalReviewsCount > 0
            ? Number((weightedSum / totalReviewsCount).toFixed(1))
            : 0;
        ...
        setStats({
          ...
          availableListings: properties.filter(
            (p) => p.status === "Approved" && p.isAvailable !== false,
          ).length,
          pendingListings: properties.filter((p) => p.status === "Pending")
            .length,
          walletBalance: walletBalance,
          ...
          occupancyRate:
            properties.length > 0
              ? Math.round(
                  (completedBookings.length / (properties.length * 30)) * 100,
                )
              : 0,
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <h3
              className={`text-3xl font-extrabold mb-1 ${stats?.walletBalance < 0 ? "text-red-400" : ""}`}
            >
              ₹{(stats?.walletBalance || 0).toLocaleString()}
            </h3>
            {stats?.walletBalance < 0 && (
              <p className="text-xs text-red-400 font-medium mb-1">
                Negative balance will be deducted from future earnings.
              </p>
            )}
        ...
        >
          ...
          <h3 className="text-3xl font-extrabold text-[#1A1A2E] mb-1">
            {stats?.totalListings}
          </h3>
          <p className="text-sm text-gray-500 font-medium">
            {stats?.availableListings} active, {stats?.pendingListings} pending
          </p>
        </div>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        if (property) {
          const ownerProfile = await userService.getUserProfile(
            property.ownerId,
          );
      ...
      >
        ...
        <div className="space-y-6 mb-8">
          {[
            ...
            <div key={item.key}>
              <label className="block text-sm font-bold text-[#1E1B4B] mb-2">
                {item.label}
              </label>
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    ...
                    onClick={() =>
                      setRatings({
                        ...ratings,
                        [item.key as keyof typeof ratings]: star,
                      })
                    }
