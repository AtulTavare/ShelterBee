...
export default function Auth() {
  ...
  const handleProceedToStep2 = (e: React.FormEvent) => {
    ...
    if (userType === 'seeker') {
      if (!firstName || !lastName || !email || !mobile || !dob || !gender || !password) {
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      ...
      if (userType === 'seeker') {
        ...
        userData.displayName = propertyHolderName;
        userData.mobile = mobile;
        userData.whatsapp = sameAsWhatsapp ? mobile : whatsapp;
        userData.gender = gender;
      }
                ...
                >
                            ...
                            <div className="space-y-2 w-full">
                              ...
                              <div className="relative flex items-center">
                                <input
                                  ...
                                  className="w-full px-4 py-3 rounded-xl bg-[#F8F9FA] border border-gray-100 focus:ring-2 focus:ring-amber-500/50 outline-none transition-all text-gray-800 pr-20"
                                />
                                {age !== null && (
                                  <div className="absolute right-2 px-3 py-1 bg-[#1A1A2E] text-white rounded-full text-xs font-bold pointer-events-none">
                                    {age} YRS
                                  </div>
                                )}
                              </div>
                  ...
                  <div className="mt-8 pt-6 border-t border-gray-100 text-center">
                    <button 
                      type="button"
                      onClick={() => {
                        navigate(`/auth?mode=${mode === 'login' ? 'register' : 'login'}`, { state: location.state });
                        setErrorMsg(null);
                      }}
