...
const getAmenityIcon = (amenity: string) => {
  ...
  if (lower.includes("laundr") || lower.includes("wash"))
...
export default function Listings() {
  ...
  useEffect(() => {
    ...
    if (sort === "price_asc") setSortBy("Price: Low to High");
    if (filter === "new_last_30_days") setFilterNewOnly(true);
  }, [location.search]);

  const maxPropertyPrice = 5000;
  ...
  const allAreas = useMemo(
    () => Array.from(new Set(properties.map((p) => p.area).filter(Boolean))),
    [properties],
  );
  ...
  useEffect(() => {
    let filtered = properties.filter((p) => {
      ...
      const propPrice = Number(p.pricePerDay) || 0;
      if (priceRange < maxPropertyPrice && propPrice > priceRange) return false;
      ...
      const maxGuestsAllowed = Number(p.guests) || 6;
      if (occupancy !== "Any" && maxGuestsAllowed < occupancy) return false;

      if (
        selectedGender !== "Any" &&
        (!p.gender || !p.gender.includes(selectedGender))
      )
        return false;
      if (selectedAmenities.length > 0) {
        const hasAllAmenities = selectedAmenities.every(
          (a) => p.amenities && p.amenities.includes(a),
        );
        if (!hasAllAmenities) return false;
      }
      if (dateRange.from) {
        const availableDate = p.availableFrom
            ...
            <div className="absolute bottom-6 md:bottom-10 left-6 md:left-16 flex gap-2 md:gap-3 z-10">
              {topProperties.map((prop, idx) => (
                <button
                  ...
                  style={
                    {
                      width: idx === currentSlide ? "32px" : "32px",
                      mdWidth: "48px",
                    } as any
                  }
                >
          ...
          {loading ? (
            ...
            <section className="space-y-8">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-4 gap-4">
                <div>
                  <h2 className="text-xl md:text-3xl font-extrabold text-on-secondary-fixed tracking-tight">
                    Top Stays for You
                  </h2>
                  <p className="text-xs md:text-base text-on-surface-variant font-medium">
                    Showing {filteredProperties.length} results matching your
                    criteria
                  </p>
                </div>
              ...
              {filteredProperties.length === 0 ? (
                <div className="bg-white rounded-[2rem] p-8 md:p-16 text-center flex flex-col items-center justify-center border border-outline-variant/10 shadow-sm">
                  ...
                  <h3 className="text-xl md:text-2xl font-extrabold text-on-secondary-fixed mb-4 tracking-tight">
                    No properties found
                  </h3>
              ...
              {filteredProperties.length > visibleCount && (
                <div className="flex justify-center py-12">
                  ...
                  >
                    ...
                    <span className="text-sm font-bold text-on-secondary-fixed uppercase tracking-widest">
